 ...
 */
 ...
 function Baraja(container, options = {}) {
     ...
     console.log("Created baraja: ", container);
     const self = this;
     ...
     function cardRemoved() {
         console.log("Items before: ", self.items);
         self.items = getItemsAsArray();
         console.log("Items after: ", self.items);
         self.itemTotal = self.items.length;
         setStack(self.items);
         if (self.itemTotal > 1) {
             fan({
                 ...
                 speed: 500,
                 range: 50 * Math.log(self.itemTotal), //100
                 center: true
             });
         }
         else {
     ...
     /**
        * Initialize element hover event handlers.
        *
        * @param {Array} items
        *   Array of HTML elements (optional).
        */
     function initHoverEvents(items = self.items) {
         items.forEach(function(item) {
             const eventHandler = function() {
                 ...
                 let newTransform = item.style.transform.replace(/translate\(.*px\)/gm, 'translate(' + x + 'px, ' + y + 'px)');
                 if (!angleAvailable) {
                     newTransform = 'translate(' + x + 'px, ' + y + 'px)';
                     setTransition(item, 'transform');
                 }
                 // console.log("Old transform: ", item.style.transform);
                 // console.log("New transform: ", newTransform);
                 // console.log("");
                 item.style.transform = newTransform;
             };
 
             item.addEventListener('mouseover', eventHandler, true);
 
             const outEventHandler = function() {
                 let newTransform = item.style.transform.replace(/translate\(.*px\)/gm, 'translate(0px)');
                 item.style.transform = newTransform;
             };
 
             item.addEventListener('mouseout', outEventHandler, true);
         });
     }
     ...
     function fan(settings = {}) {
         ...
         if (settings.origin.minX && settings.origin.maxX) {
             ...
             console.log("Fanning 2: ", self.itemTotal, self.items);
             self.items.forEach(function(item) {
             ...
             console.log("Fanning: ", self.itemTotal, self.items);
             self.items.forEach(function(item) {
     ...
     this.cardRemoved = function(item) {
         cardRemoved(item);
     }
