...
function Baraja(container, options = {}) {
  ...
  function initHoverEvents(items = self.items) {
    items.forEach(function (item) {
      ...
      const outEventHandler = function () {
        if (item.classList.contains("selected-wild")) {
            console.log("Is the wild class! Do not stop the hovering!");
            return;
        }
        let newTransform = item.style.transform.replace(
          /translate\(.*px\)/gm,
  ...
  this.setSelectedWild = function (i) {
    setSelectedWild(i);
  };

  this.unselectWild = function(i) {
    unselectWild(i);
  }
}
